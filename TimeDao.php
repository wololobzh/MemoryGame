<?php
/*
* SCRIPTS A EXECUTER SUR LE SERVEUR DE BASE DE DONNEES (via phpMyAdmin).
*
...
//Si TimeDao est appelé avec le paramètre 'time' alors on est dans le cas ou l'on désire enregistrer un temps.
if (isset($_POST['time']))
{
...
{
	//Récupération d'un tableau avec les resultats
	$meilleursTemps = getTheThreeBestTime();
	
	echo("Meilleurs scores <br/>");
	
	//Affichage de chaque résulat.
...
function getTheThreeBestTime()
{
	...
	#La base de données est un élément exterieur que l'on ne maitrise pas.
    try
    {
		#Récupération d'une variable représentant une connexion à la bdd.
		$cnx = getConnection();
		
		#Création d'une requete contenant un paramètre
...
/**
* Permet d'inserer un temps dans la table.
*/
